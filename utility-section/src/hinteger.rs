/// Decode the ersatz-Huffman integer represented by the given slice. Inversely:
///
...
///
/// Returns a tuple: the decoded value, and the count of bytes that represented
/// it. Or None if the decoded value would exceed M, or if decoding ran out of
/// bytes.
pub fn hinteger_explicit_maximum<const M: usize>(encoded: &[u8]) -> Option<(usize, usize)> {
    let max_accum_okay: usize = M >> 7;
    let mut accum = 0usize;
    for (offset, b) in encoded.iter().enumerate() {
        if *b < 128u8 {
            return Some(((accum << 7) + (*b as usize), offset + 1));
        }
        let val = b & 0x7f;
...
pub fn hinteger(encoded: &[u8]) -> Option<(usize, usize)> {
    hinteger_explicit_maximum::<{ usize::MAX }>(encoded)
}
...
mod tests {
    ...
    fn zero() {
        const B: [u8; 1] = *b"\0";
        let (val, offset) = hinteger(&B).unwrap();
    ...
    fn padded_zero() {
        ...
        let (val, addr) = hinteger(&B).unwrap();
        assert_eq!(val, 0usize);
